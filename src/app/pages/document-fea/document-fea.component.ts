...
@Component({
  selector: 'app-document-fea',
  templateUrl: './document-fea.component.html',
  styleUrls: ['./document-fea.component.css']
})
export class DocumentFEAComponent  implements OnInit{
  showModal = false;
  ...
  obtenerDocumentos(){
    this.documentService.obtenerDatos().subscribe((data)=>{
      this.documentosPendientes = data.filter((documento:any) =>
        documento.typeDocument === 'Documento FEA' 
      )
