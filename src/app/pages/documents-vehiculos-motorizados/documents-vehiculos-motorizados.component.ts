...
@Component({
  selector: 'app-documents-vehiculos-motorizados',
  templateUrl: './documents-vehiculos-motorizados.component.html',
  styleUrls: ['./documents-vehiculos-motorizados.component.css']
})
export class DocumentsVehiculosMotorizadosComponent implements  OnInit{
  ...
  obtenerDocumentos(){
    this.documentService.obtenerDatos().subscribe((data)=>{
      this.documentosPendientes = data.filter((documento:any) =>
        documento.typeDocument === 'Promesa'
      )
